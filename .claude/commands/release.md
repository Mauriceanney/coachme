...
### 4. Update create-agentic-saas Version

Sync the version in create-agentic-saas/package.json to match the new main version.

```bash
# Read new version from main package.json
NEW_VERSION=$(node -p "require('./package.json').version")

# Update create-agentic-saas/package.json version
cd create-agentic-saas && npm version $NEW_VERSION --no-git-tag-version --allow-same-version && cd ..
```

**Display:**
```
Updating create-agentic-saas version: {old} -> {new}
```

**On failure:**
```
Error: Failed to update create-agentic-saas version.
{error message}
...
Sync the main project to the create-agentic-saas template directory.

```bash
node create-agentic-saas/scripts/sync-templates.js
```
...
```bash
# Stage changed files (including create-agentic-saas)
git add package.json CHANGELOG.md create-agentic-saas/
...
- Synced create-agentic-saas template
...
Publish the create-agentic-saas package to npm.
...
**Publish:**
```bash
cd create-agentic-saas && npm publish --access public && cd ..
```

**On publish success:**
```
Published create-agentic-saas@{version} to npm
```
...
The create-agentic-saas package was not published.
Manual publish required: cd create-agentic-saas && npm publish
```
...
- create-agentic-saas/package.json (version)
- create-agentic-saas/template/ (synced)
...
- npm: FAILED - manual publish required
  Run: cd create-agentic-saas && npm publish
...
3. Update create-agentic-saas/package.json:
   - "version": "{old}" -> "version": "1.2.0"

4. Sync template:
   - Would run: node create-agentic-saas/scripts/sync-templates.js
...
10. Publish to npm:
    - Would run: cd create-agentic-saas && npm publish --access public
    - Package: create-agentic-saas@1.2.0
...
| create-agentic-saas/package.json missing | Error, abort release |
